...
class BackendApi {
  ...
  static void printWarning(dynamic text) {
    final dynamic d = prettifyJsonEncode(text);
    print('\x1B[33m$d\x1B[0m');
  }

  static void printError(dynamic text) {
    final dynamic d = prettifyJsonEncode(text);
    print('\x1B[31m$d\x1B[0m');
  }

  Future<Map> fetchHomePageData() async {
    Map result = {};
    try {
      final Uri url = Uri.parse('${BackendApi().ApiUrl}/home'); // Convert the URL to a Uri object
  ...
  Future<Map> fetchSongDetails(String slug) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        final Map data = json.decode(res.body) as Map;
        print(data);
        final Map result = await FormatResponse.formatHomePageData(data);
      }
